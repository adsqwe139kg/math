...
if __name__ == '__main__':
    ...
    @app.route('/search')
def search():
    # Отримуємо те, що користувач ввів у пошуковий рядок
    query = request.args.get('query', '').strip().lower()
    
    results = {}
    if query:
        # Шукаємо збіги за назвою теми або її вмістом у твоєму TOPICS
        for topic_id, topic_data in TOPICS.items():
            title = topic_data.get('title', '').lower()
            # Якщо у TOPICS структура простіша, можна шукати просто по назвах
            if query in title or query in topic_id:
                results[topic_id] = topic_data
